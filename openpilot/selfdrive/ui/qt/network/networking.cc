...
AdvancedNetworking::AdvancedNetworking(QWidget* parent, WifiManager* wifi): QWidget(parent), wifi(wifi) {
  ...
  QObject::connect(wifiMeteredToggle, &MultiButtonControl::buttonClicked, [=](int id) {
    ...
    MeteredType mt = MeteredType::UNKNOWN;
    if (id == NM_METERED_YES) {
      mt = MeteredType::YES;
    } else if (id == NM_METERED_NO) {
      mt = MeteredType::NO;
    }
    auto pending_call = wifi->setCurrentNetworkMetered(mt);
    if (pending_call) {
...
WifiItem *WifiUI::getItem(int n) {
  ...
  if (!item->parentWidget()) {
    ...
    QObject::connect(item, &WifiItem::forgotNetwork, [this](const Network network) {
