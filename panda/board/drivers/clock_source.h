...
#ifdef STM32F4
// STM32F4 (dos): TIM1-only camera strobe, 3 channels on TIM1.
// 2026 moved the H7 strobe to a TIM1+TIM8 master/slave pair, but dos hardware was
// validated with this TIM1-only timing and its peripherals_init() never enables
// the TIM8 clock, so keep the proven path here.
...
void clock_source_init(bool enable_channel1) {
  ...
  register_set(&(TIM1->CCR3), (CLOCK_SOURCE_PULSE_LEN_MS*10U), 0xFFFFU);      // Compare 3 value
